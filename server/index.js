...
import { kerniumKnowledge } from './kernium.js';
...
const server=http.createServer(async(req,res)=>{
 try {
  ...
  if(url.pathname==='/api/kernium/manual' && ['GET','HEAD'].includes(req.method)) {
   const content=await readFile(new URL('../data/kernium/manual.pdf',import.meta.url));
   res.writeHead(200,{'Content-Type':'application/pdf','Content-Disposition':'inline; filename="Jungheinrich-EFG-213-320-51099986.pdf"','Content-Length':content.length});
   return res.end(req.method==='HEAD'?undefined:content);
  }
  if(url.pathname==='/api/status' && req.method==='GET') return json(res,200,await aiStatus(config));
  if(['/api/chat','/api/kernium/chat'].includes(url.pathname) && req.method==='POST') {
   let body='', size=0;
   ...
   try{
    const knowledge=url.pathname==='/api/kernium/chat'?kerniumKnowledge(data.message,history):null;
    return json(res,200,await diagnose(data.message,history,{...config,knowledge}));
  ...
  const routes={'/':'/index.html','/impresora':'/index.html','/impresora/':'/index.html','/kernium':'/kernium/index.html','/kernium/':'/kernium/index.html'};
  const target=path.resolve(root,'.'+(routes[pathname]||pathname));
