...
const server=http.createServer(async(req,res)=>{
 try {
  ...
  if(url.pathname==='/api/status' && req.method==='GET') return json(res,200,{mode:process.env.OPENAI_API_KEY?'live':'demo',model:process.env.OPENAI_API_KEY?(process.env.OPENAI_MODEL||'gpt-4.1-mini'):null});
  if(url.pathname==='/api/chat' && req.method==='POST') {
   ...
   try{return json(res,200,await diagnose(data.message,history,{apiKey:process.env.OPENAI_API_KEY,model:process.env.OPENAI_MODEL||'gpt-4.1-mini'}));}catch(error){return json(res,502,{error:error.message});}
  }
...
server.listen(port,'127.0.0.1',()=>console.log(`LAYER → http://localhost:${port} · ${process.env.OPENAI_API_KEY?'IA conectada':'Demo documentada'}`));
